...
const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      const todoStore = db.createObjectStore(todoStoreName, { keyPath: "id" });
      todoStore.createIndex("sort", "sort", { unique: true });
      todoStore.createIndex("date", "date", { unique: false });
      db.createObjectStore(userStoreName, { keyPath: "userId" });
    };
...
const login = (userId: string, password: string): Promise<string> => {
  return new Promise(async (resolve, reject) => {
    try {
      const db = await openDatabase();
      ...
      if (userId !== null && password !== null) {
        const addRequest = store.get(userId);

        addRequest.onsuccess = () => {
          const getRequest = store.get(userId);
          getRequest.onsuccess = () => {
            const userDate = getRequest.result;
            const passwordHash = CryptoJS.PBKDF2(password, userDate.salt, {
              keySize: 512 / 32,
              iterations: 10000,
            }).toString();

            if (passwordHash !== userDate.password) {
              reject();
            }
...
export const handlers = [
  ...
  rest.post("/login", async (req, res, ctx) => {
    ...
    const token: string = await login(userId, password);
    ...
    const cookie = serialize("jwt", token, cookieOptions);
    return res(ctx.status(200), ctx.json({ success: true }), ctx.cookie("set-cookie", cookie, cookieOptions));
  }),

  rest.post("/signup", async (req, res, ctx) => {
    const { userId, password } = await req.json();
    const token = await signup(userId, password);
