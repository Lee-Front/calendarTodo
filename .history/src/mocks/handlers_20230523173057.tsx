...
const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      todoStore.createIndex("sort", ["userId", "sort"], { unique: false });
...
const addTodo = (date: string, userId: string): Promise<IDBValidKey> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log("1");
      const getMaxSortRequest = store.index("sort").openCursor([userId, date], "prev");
      console.log("2");
      getMaxSortRequest.onsuccess = async (event) => {
        console.log("3");
        const cursor = await (event.target as IDBRequest<IDBCursorWithValue>).result;
