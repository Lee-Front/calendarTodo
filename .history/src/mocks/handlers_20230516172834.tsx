import { RequestRedirect, rest } from "msw";
...
const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      todoStore.createIndex("date", ["date", "userId"], { unique: false });
      todoStore.createIndex("compositeIndex", ["date", "userId"], { unique: false });
...
const searchCalendar = (startDate: string, endDate: string, userId: string): Promise<SearchCalendarResult> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const tx = db.transaction(todoStoreName, "readonly");
      const store = tx.objectStore(todoStoreName);
      const range = IDBKeyRange.bound([new Date(startDate), userId], [new Date(endDate), userId]);
      const index = store.index("compositeIndex");

      const request = store.index("compositeIndex").openCursor(range);
