    ...
    try {
      ...
      store.index("userId").get(userId).onsuccess = (event) => {
        ...
        if (!result) {
          ...
          const accessToken: Promise<string> = createJwt(userId, accessExp, accessKey);
          const refreshToken: Promise<string> = createJwt(userId, refreshExp, refreshKey);
...
export const handlers = [
  rest.post("/refreshToken", async (req, res, ctx) => {
    ...
    try {
      ...
      const token: string = await createJwt(userId, accessExp, accessKey);
      const accessTokenCookie: string = serialize("accessToken", token, cookieOptions);
