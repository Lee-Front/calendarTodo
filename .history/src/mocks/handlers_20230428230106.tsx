...
const addTodo = (date: Date): Promise<IDBValidKey> => {
  return new Promise(async (resolve, reject) => {
    try {
      const db = await openDatabase();
      const tx = db.transaction(storeName, "readwrite");
      const store = tx.objectStore(storeName);
      const createDate = typeof date === "string" ? new Date(date) : date;
...
const searchTodos = (searchDate: string): Promise<Todo[]> => {
  return new Promise(async (resolve, reject) => {
    try {
      const db = await openDatabase();
      ...
      if (searchDate !== null) {
        ...
        console.log("searchDate: ", new Date(searchDate));
...
const searchCalendar = (
  startDate: string,
  endDate: string
): Promise<{ [key: string]: { total: number; completed: number } }> => {
  return new Promise(async (resolve, reject) => {
    try {
      const db = await openDatabase();
      const tx = db.transaction(storeName, "readonly");
      const store = tx.objectStore(storeName);
      const range = IDBKeyRange.bound("2023-03-01", "2023-03-03");
