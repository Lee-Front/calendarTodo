...
const authInterceptor = (accessToken: string) => (req: any, res: any, ctx: any) => {
  // Set the access token in the Authorization header
  return req(ctx.set("Authorization", `Bearer ${accessToken}`));
};

const refreshTokenInterceptor = () => async (req: any, res: any, ctx: any) => {
  let accessToken = "";
  if (!accessToken) {
  ...
  // Set the access token in the Authorization header and proceed with the request
  return req(authInterceptor(accessToken)(req, res, ctx));
};
...
export const handlers = [
  ...
  // 포스트 목록
  rest.get("/", refreshTokenInterceptor(), async (req, res, ctx) => {
