    ...
    try {
      ...
      request.onsuccess = function (event) {
        ...
        if (cursor) {
          ...
          const monthTodosInfo = new Map();
          dates.map((item: any) => {
            const completedTodos = todosByDate[item].filter(
              (v: Todo) => v.check
            );
            monthTodosInfo.set(item, {
              total: todosByDate[item].length,
              completed: completedTodos.length,
            });
            console.log("todosByDate[item]: ", todosByDate[item]);
            console.log("completedTodos: ", completedTodos);
            // todosByDate[item].map((todo: Todo) => {
            //   monthTodosInfo.set(todo.date, { total: 0, completed: 0 });
            // });
          });
          console.log("test : ", monthTodosInfo.get("2023-4-1"));
        }
      };
    } catch (err) {
      console.error(err);
