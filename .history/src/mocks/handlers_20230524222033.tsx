        ...
        store.index("date").count([userId, date]).onsuccess = (countEvent) => {
          ...
          if (count === 0) {
            ...
            const test = new Promise((resolve) => {
              styleStore.index("composite").get([userId, date]).onsuccess = async (event) => {
                console.log({ userId, date });
                const styleCursor = (event.target as IDBRequest).result;
                console.log("styleCursor : ", styleCursor);
                resolve(styleCursor);
              };
            });
            console.log("dateStr: ", dateStr);
            console.log("date : ", date);
            // createDate :  Sun Apr 02 2023 00:00:00 GMT+0900 (한국 표준시) {}
            //               Tue May 02 2023 00:00:00 GMT+0900 (한국 표준시) {}
            // userId : "wkdrmadl3"

            userId: "wkdrmadl3";
            Promise.all([test]).then((test) => {
              console.log(test);
              resolve();
            });
          }
