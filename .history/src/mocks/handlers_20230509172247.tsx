...
const todoStore = "todos_store";
const userStore = "todos_store";
...
const openDatabase = (storeName: string): Promise<IDBDatabase> => {
...
const addTodo = (date: string): Promise<IDBValidKey> => {
  return new Promise(async (resolve, reject) => {
    try {
      const db = await openDatabase(todoStore);
...
const signup = (id: string, password: string): Promise<Todo[]> => {
  return new Promise(async (resolve, reject) => {
    try {
      const db = await openDatabase(userStore);
...
export const handlers = [
  ...
  rest.post("/signup", async (req, res, ctx) => {
    ...
    const token = await signup(id, password);
