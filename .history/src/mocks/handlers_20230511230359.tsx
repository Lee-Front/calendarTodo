...
const validateToken = async (token: string) => {
  ...
  const secretKey = encoder.encode(JWT_SECRET);
  try {
    const jwt = await jwtVerify(token, secretKey, { issuer: "calendar-todo-app" });
    console.log(jwt);
    //return jwt.payload["urn:calendarTodo:claim"];
  } catch (e) {
    console.log("asd");
    return false;
  }
...
export const handlers = [
  ...
  rest.post("/accessToken", async (req, res, ctx) => {
    ...
    const cookie = serialize("accessToken", accessToken, cookieOptions);
    return res(ctx.status(200), ctx.cookie("accessToken", cookie));
  }),
  ...
  rest.get("/calendar", async (req, res, ctx) => {
    const check = validateToken("aa");
    console.log(parse(req.cookies.accessToken));
    console.log("req : ", req);
