...
const requestToPromise = (request: IDBRequest) => {
...
const addTodo = async (todo: string) => {
  ...
  const cursor = await requestToPromise(getMaxIdRequest);
  ...
  getMaxIdRequest.onsuccess = (event) => {
    ...
    getMaxSortRequest.onsuccess = (event) => {
      ...
      addRequest.onsuccess = async () => {
        const getRequest = await store.get(addRequest.result);
...
const updateTodo = (todo: Todo): Promise<void> => {
  return new Promise(async (resolve, reject) => {
    try {
      const db = await openDatabase();
      const tx = db.transaction(storeName, "readwrite");
