    ...
    try {
      ...
      store.index("sort").openCursor([userId, createDate], "prev").onsuccess = async (event) => {
        ...
        store.add(newTodo).onsuccess = async (addEvent) => {
          const addCursor = await (addEvent.target as IDBRequest).result;
          store.get(addCursor).onsuccess = async (getEvent) => {
            const todoCursor = await (getEvent.target as IDBRequest).result;
            resolve(todoCursor);
          };
        };
      };
...
const searchTodos = (date: string, userId: string): Promise<Todo[]> => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      store.index("date").getAll(IDBKeyRange.only(searchDate)).onsuccess = (event) => {
