    ...
    try {
      ...
      if (userId !== null && password !== null) {
        ...
        getRequest.onsuccess = () => {
          ...
          const accessToken = createJwt(userId, "5s", accessKey);
          const refreshToken = createJwt(userId, "5s", refreshKey);
...
export const handlers = [
  rest.post("/refreshToken", async (req, res, ctx) => {
    ...
    const test1 = refreshToken?.split("=")[1]?.split(";")[0] ?? "";
    const test2 = getCookie(refreshToken ?? "");

    console.log("test1 :", test1);
    console.log("test2 : ", test2);
    const jwt = await jwtVerify(test1, refreshKey, {
      issuer: "calendar-todo-app",
      audience: "https://lee-front.github.io",
      algorithms: ["HS256"],
    });
    console.log("jwt : ", jwt);
    const token = await createJwt(test1, accessExp, accessKey);
