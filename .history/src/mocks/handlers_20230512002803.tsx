...
// 토큰 유효시간을 지정할 변수
const createJwt = async (userId: string, tokenExp: number): Promise<string> => {
...
const validateToken = async (token: string) => {
  try {
    ...
    //   const test = new Date(1683825746 * 1000);
    ...
    try {
      ...
      if (userId !== null && password !== null) {
        ...
        getRequest.onsuccess = () => {
          ...
          const token = createJwt(userId, 60);
...
export const handlers = [
  rest.post("/refreshToken", async (req, res, ctx) => {
    ...
    const token = await createJwt(refreshToken, 60 * 60 * 2);
