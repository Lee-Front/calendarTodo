...
export const handlers = [
  rest.post("/refreshToken", async (req, res, ctx) => {
    ...
    try {
      ...
      if (error.code === "ERR_JWT_EXPIRED") {
        ...
        return res(ctx.status(301), ctx.json({ message: "Expired token" }), ctx.set("Location", "/login"));
      }
    }
  }),
  ...
  rest.get("/login", (req, res, ctx) => {
    // /login 으로 redirect되는 소스
    console.log("11");
    return res(ctx.status(307), ctx.set("Location", "/login"));
  }),
