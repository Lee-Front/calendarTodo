    ...
    try {
      ...
      store.delete(id).onsuccess = () => {
        ...
        store.index("date").count([userId, date]).onsuccess = (countEvent) => {
          const count: number = (countEvent.target as IDBRequest).result;
        ...
        if (result) {
          ...
          const requests = dates.map((item) => {
            ...
            return new Promise((resolve) => {
              const createDate: Date = todosByDate[item][0]?.date;
...
export const handlers = [
  ...
  rest.get("/calendar", async (req, res, ctx) => {
    ...
    const startDate: string | null = req.url.searchParams.get("startDate");
    const endDate: string | null = req.url.searchParams.get("endDate");
    const userId: string | null = req.url.searchParams.get("userId");

    if (startDate && endDate && userId) {
      const todos: SearchCalendarResult = await searchCalendar(startDate, endDate, userId);
      return res(ctx.status(200), ctx.json(todos));
    } else {
      return res(ctx.status(400), ctx.json({ message: "Invalid date range" }));
    }
  ...
  rest.post("postItStyle", async (req, res, ctx) => {
    ...
    const { date, userId, color }: { date: string; userId: string; color: { r: number; g: number; b: number } } =
      await req.json();
    const response = await addPostItStyle(date, userId, color);
    return res(ctx.status(201), ctx.json(response));
  }),
];
