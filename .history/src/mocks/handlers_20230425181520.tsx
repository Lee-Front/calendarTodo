...
const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      store.createIndex("dateIndex", "dateIndex", { unique: false });
    };
...
const searchTodos = (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let request;
      //const request = index.getAll(searchDate);

      if (searchDate !== null) {
        request = index.getAll(
          IDBKeyRange.bound(searchDate, `${searchDate}T23:59:59`)
        );
        //request = index.getKey(searchDate);
      } else {
