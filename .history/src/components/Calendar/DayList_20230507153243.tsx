...
import { DayInfo, DateRange, DayListProps, TodoInfo, CalendarMonth, DayOfWeek } from "../../types/calendarTypes";
...
const DayList = ({ currentDate }: DayListProps) => {
  ...
  const dayColor = (month: CalendarMonth, dayOfWeek: DayOfWeek, { currentDate }: DayListProps) => {
    const isDifferentMonth = month !== currentDate.getMonth();
        ...
        .map((_, index) => (
          <CalendarWeek key={index}>
            {dayInfos.slice(index * 7, index * 7 + 7).map((dayInfo) => {
              const isDifferentMonth = dayInfo.month !== currentDate.getMonth();
              const textColor = "";
              ...
              return (
                ...
                >
                  {dayInfo.todoInfo ? (
                    <PostIt dayInfo={dayInfo} isDifferentMonth={isDifferentMonth}></PostIt>
                  ) : (
                    dayInfo.day
                  )}
                </CalendarDay>
...
const CalendarDay = styled.div<{ isDifferentMonth: boolean; dayOfWeek: number }>`
  position: relative;
  font-size: 2rem;
  flex: 1;
  padding: 0.5rem;
  aspect-ratio: 1;
  color: ${(props) => {
    let color = props.isDifferentMonth ? "#8e8c8d" : "#4c4c4c";
    if (props.dayOfWeek === 0) {
      color = props.isDifferentMonth ? "#d8999d" : "#e85c5e";
    } else if (props.dayOfWeek === 6) {
      color = props.isDifferentMonth ? "#8ba8d2" : "#6383bc";
    }
    return color;
  }};
