          ...
          <CalendarWeek key={index}>
            {dayInfos.slice(index * 7, index * 7 + 7).map((dayInfo) => {
              ...
              return (
                ...
                >
                  {dayInfo.todoInfo ? (
                    <PostIt dayInfo={dayInfo} backgroundColor={background-color: ${(props) => (props.month !== props.currentDate.getMonth() ? "#fff0b3" : "#ffea92")};}></PostIt>
                  ) : (
                    dayInfo.day
                  )}
                </CalendarDay>
...
const CalendarDay = styled.div<{ month: number; dayOfWeek: number; currentDate: Date }>`
  position: relative;
  font-size: 2rem;
  flex: 1;
  padding: 0.5rem;
  aspect-ratio: 1;
  color: ${(props) => dayColor(props.month, props.dayOfWeek, props.currentDate)};
  background-color: ${(props) => (props.month !== props.currentDate.getMonth() ? "#fff0b3" : "#ffea92")};
`;
