    ...
    <MonthSelectorContainer>
      ...
      <CalendarTitle>
        ...
        <CalendarTextWrapper>
          ...
          <CalendarTextAnimation size={4}>{currentDate.getMonth() + 1}월</CalendarTextAnimation>
        </CalendarTextWrapper>
      </CalendarTitle>
      <CalendarMonthButton onClick={() => handleMonthChange(currentDate, +1)}>
...
const CalendarTextWrapper = styled.div`
  display: flex;
  flex-direction: column;
  position: relative;
`;
...
`;
const CalendarTextAnimation = styled.span<{ size: number }>`
  position: absolute;
  font-size: ${(props) => props.size}rem;
  line-height: ${(props) => props.size}rem;
  //top: ${(props) => (props.isAnimation ? "-100%" : "0")};
  //left: 0;
  transition: all 0.3s ease-in-out;
`;
