...
import { CalendarDayInfo, CalendarRange, DayListProps, TodoInfo } from "../../types/calendarTypes";
...
const DayList = ({ currentDate }: DayListProps) => {
  ...
  const createCalendar = (curDate: Date): { calendarDayInfos: CalendarDayInfo[]; calendarInfo: CalendarRange } => {
          ...
          <CalendarWeek key={index}>
            {dayInfos.slice(index * 7, index * 7 + 7).map((dayInfo) => {
              ...
              return (
                ...
                >
                  {dayInfo.todoInfo ? (
                    <PostIt todoInfo={dayInfo.todoInfo} isDifferentMonth={dayInfo.month !== currentDate.getMonth()></PostIt>
                  ) : (
                    dayInfo.day
                  )}
                </CalendarDay>
